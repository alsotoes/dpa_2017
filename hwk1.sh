...
xargs -n1 | awk 'NR%58{printf "%s\t",$0;next}7')"'\n/gm' |\
tee >(awk -F $'\t' '{ a[$57]++; b[$57]=$31} END { for (i in a) print i,","a[i],","b[i]| "sort -nk 1"}' |(echo "Dia,Eventos,GoldStein" ; tail -n +2) |csvsql --db sqlite:///mexico.db --tables mexico_ts --insert ) |\
